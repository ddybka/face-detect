...
vs = VideoStream(src=0, usePiCamera=False, resolution=FRAMESIZE, framerate=32).start() # на винде
#vs = VideoStream(src=0, usePiCamera=True, resolution=FRAMESIZE, framerate=32).start() # на расбери
...
while True:
    ...
    for hue in HUES:
        ...
        for c in cnts:
            ...
            draw.text((cX, cY), shapename+" ("+str(cX)+","+str(cY)+")", font=FONT)
